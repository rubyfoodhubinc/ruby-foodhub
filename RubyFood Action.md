...
**Bug found and fixed while testing the viewer role (commit `40fecb3`):** the "Users" nav tab was visible to every non-owner account (managers included) since named admin accounts shipped. `session.role !== 'owner'` correctly set the `hidden` attribute, but `.nav-item { display: block }` ties `[hidden] { display: none }` on CSS specificity, and the page's own stylesheet wins that tie. No data exposure — the tab 403'd if clicked (admin-users.js is owner-only) — but visibly wrong on the exact screen an App Review reviewer sees first. Fixed with `.nav-item[hidden] { display: none }` (one extra selector, wins deterministically).

**Admin iOS build 3** (1.0 (3), not 1 or 2) archived and uploaded — this is the one attached to the public submission; it has both the viewer-aware frontend and the nav fix. Verified end-to-end in the Simulator signed in as the viewer account: banner shows, Users tab gone, action buttons hidden.

**Migration 015 confirmed applied**: the owner created the viewer account (moneymarttvllc@gmail.com, role `viewer`) through the live Admin UI and successfully signed in with it — a `role='viewer'` row could not exist against the pre-015 constraint, so this is direct proof the migration ran, not just an assumption.

**Both apps submitted for App Review by the owner** as of this update, using the listing copy, screenshots, and App Review notes prepared in `~/Desktop/RubyFoodHub-iOS-builds/Ruby FoodHub — App Store Details.docx`. Wholesale on build 1.0(1), Admin on build 1.0(3). Standard Apple review turnaround is 24–48 hours — next session should check both apps' status in App Store Connect before assuming anything else is wrong.

**Still open:** 4 of the 6 additional TestFlight testers requested were never created (`rubynavinctech@icloud.com`, `willigbekhai@gmail.com`, `dianadokpesi@icloud.com`, `willdokpesi@gmail.com`); 2 were created (`ladydokpesi@icloud.com`, `dianadokpesi@gmail.com`) but got Marketing+Sales roles rather than Marketing-only and were never added to a TestFlight group. Wholesale app still has the stray macOS platform in "Prepare for Submission" — harmless, just clutter.
